...
> [!NOTE]
> - Some of the `Software`'s repository policies and information may change over time without any warning. It is recommended that you read these policies once every 6-12 months, and that you update your code to the relevant policies, such as updating your parts of codes in the `Software` repository to abide to new security standards, etc. However you will not be given a warning if you forget as the `Software` team doesn't know if you are a one-time contributor or a full-time contributor, or whether you suddenly leave.
